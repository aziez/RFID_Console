...
class Splashscreen:
    ...
    def aturWindow(self):
        lebar = 300
        tinggi = 100
        setTengahLebar = (self.parent.winfo_screenwidth()-lebar)//2
        setTengahTinggi = (self.parent.winfo_screenheight()-tinggi)//2

        self.parent.geometry("%ix%i+%i+%i" % (lebar, tinggi, setTengahLebar, setTengahTinggi))

        Label(self.parent, text="Loading", bg="#a1dbcd", fg="white", font=("Helvetica",20), width=9, height=2).pack(side=TOP, anchor=N, fill=X)
        s = ttk.Style()
        s.theme_use('alt')
        s.configure("red.Horizontal.TProgressbar", background='#444', foreground="white", realief=FLAT, bd=0, height=2)
